...
def export_browser_assets(solids):
    ...
    chunks = bytearray()
    for solid in solids:
        all_verts = [v for part in solid.parts for v in part.verts]
        ...
        for part in solid.parts:
            offset = len(chunks)
            chunk = bytearray()
            for (x, y, z), (u, v) in zip(part.verts, part.uvs):
    ...
    for name in ("anatomy.bin", "anatomy.json"):
        p = SRC / name
        manifest_data[name] = {
